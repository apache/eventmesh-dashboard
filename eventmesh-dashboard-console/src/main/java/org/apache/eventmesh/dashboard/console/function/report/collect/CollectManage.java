 ...
 *  1. TODO 如果多个集群共享 多个注册中心  然后有多个 prometheus，会出现重复采集问题。 <p>
 *          所以共享 meta 集群不允许 使用 PROMETHEUS 模式。只允许使用 EVENTMESH  <p>
 *  2. meta 集群 创建的时候可以默认创建 PROMETHEUS   <p>
 *  3. meta 集群节点发生更新的时候，也需要更新 PROMETHEUS  <p>
 *  4. 提供重启功能  <p>
...
public class CollectManage {
    ...
    public void unregister(String url, ClusterMetadata clusterMetadata) {
        this.collectExporterMap.remove(url);
    }
    ...
    private  class SyncAndHandlerData{
        ...
        public void handlerData() {
            ...
            this.runtimeMetadataList.forEach(runtimeMetadata -> {
                ...
                if(Objects.isNull(abstractMetadataCollect)){

                }
                Map<Long,RuntimeMetadata> runtimeMetadataHashMap = runtimeMetadataMap.computeIfAbsent(runtimeMetadata.getClusterId(), k -> new HashMap<>());
                if(runtimeMetadata.isInsert() || runtimeMetadata.isUpdate()){
