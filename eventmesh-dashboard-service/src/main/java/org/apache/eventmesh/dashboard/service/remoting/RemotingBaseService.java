...
/**
 * 需要所有接口提供 结果直接返回 与 GlobalResult 返回两种方案。
 * 第一种方案： 在 结果直接返回 调用 resultObjectToGlobalResult 方法，</p>
 *            在 GlobalResult 调用 globalResultToResultObject 方法 </p>
 *            子接口可以重写一类接口或全部接口
 *            问题： globalResultToResultObject 打印日志不方便</p>
 *
 *            放弃</p>
 * 第二种方案： GlobalResult 返回 EMPTY_RESULT，表示 没有实现。</p>
 *            结果直接返回 如何确定 没有实现 </p>
 *
