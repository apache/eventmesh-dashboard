...
public class RemotingManage {
     ...
     */
    private final Map<Long, ColonyDO<ClusterDO>> colonyDOMap = new HashMap<>();
    ...
    public void registerColony(ColonyDO<ClusterDO> colonyDO) throws Exception {
        if (loading.get()) {
            return;
        }
    ...
    public RemotingWrapper getMainRemotingWrapper(ColonyDO<ClusterDO> colonyDO) {
        Long clusterId =
    ...
    public ColonyDO getMainColonyDO(ColonyDO<ClusterDO> colonyDO) {
        Long clusterId =
            colonyDO.getClusterDO().getClusterInfo().getClusterType().isMainCluster() ? colonyDO.getClusterId() : colonyDO.getSuperiorId();
        return colonyDOMap.get(clusterId);
    }

    public void unregister(ColonyDO<ClusterDO> colonyDO) {
        remotingWrapperMap.remove(colonyDO.getClusterId());
    }

    public void loadingCompleted() throws Exception {
        this.loading.set(false);
        for (ColonyDO<ClusterDO> colonyDO : colonyDOMap.values()) {
            if (colonyDO.getClusterDO().getClusterInfo().getClusterType().isMainCluster()) {
    ...
    public boolean isClusterTrusteeshipType(Long clusterId, ClusterTrusteeshipType clusterTrusteeshipType) {
        ColonyDO<ClusterDO> colonyDO = this.colonyDOMap.get(clusterId);
    ...
    private List<RemotingWrapper> filterer(ClusterType clusterType, ClusterTrusteeshipType... clusterTrusteeshipTypes) {
        ...
        for (RemotingWrapper remotingWrapper : remotingWrapperMap.values()) {
            ColonyDO<ClusterDO> colonyDO = remotingWrapper.getColonyDO();
            ClusterMetadata clusterMetadata = colonyDO.getClusterDO().getClusterInfo();
    ...
    public <T> T request(RemotingRequestWrapper remotingRequestWrapper, List<RemotingWrapper> remotingWrapperList) {
        ...
        Class<Global2Request> globalRequestClass = null;
        Class<?> executeClass = null;
        if (superclass instanceof ParameterizedType) {
            ParameterizedType parameterizedType = (ParameterizedType) superclass;
            Type[] actualTypeArguments = parameterizedType.getActualTypeArguments();
            executeClass = (Class<Global2Request>) actualTypeArguments[0];
            globalRequestClass = (Class<Global2Request>) actualTypeArguments[1];
        }
        ...
        for (RemotingWrapper remotingWrapper : remotingWrapperList) {
            try {
                Global2Request global2Request = globalRequestClass.newInstance();
                global2Request.setClusterId(remotingWrapper.getColonyDO().getClusterId());
                GlobalResult<Object> globalResult = remotingRequestWrapper1.request(global2Request, executeClass);
    ...
    public class RemotingServiceHandler implements InvocationHandler {
        ...
        private Map<ClusterType, Map<Class<?>, Object>> clusterTrusteeshipType = new HashMap<>();
        ...
        public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
            ...
            Global2Request global2Request = (Global2Request) args[0];
            Long clusterId = global2Request.getClusterId();
            // ClusterDO
